...
public class ProdetaliesActivity extends AppCompatActivity {
private Button addtocart ;
    private ImageView proimg;

    private TextView proname , proprice,proderc,type0 ,proowner;
private String prodid="" , state="normal" ,proo="",imge ="";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_prodetalies);

        type0= findViewById(R.id.btnnum);
        proimg= findViewById(R.id.proimg);
        proname=findViewById(R.id.prodname1111);
        ...
        addtocart.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (state.equals("order shipped")){
                    Toast.makeText(ProdetaliesActivity.this, "you can add purchase more products , once your order is shipped or confirmed",Toast.LENGTH_LONG).show();
                }
                else {
                    addtocartlist();
                }
...
savecurrentdata=simpleDateFormat.format(calendar.getTime());
  ...
  cartmap.put("im",   imge);
     ...
     .child(Prevalent.currentonlienuser.getPhone()).child("Products").child(prodid).updateChildren(cartmap).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()){
                    Toast.makeText(ProdetaliesActivity.this,"added to cart list", Toast.LENGTH_LONG).show();
                    Intent intent= new Intent(ProdetaliesActivity.this, HomeActivity.class);
    ...
    private void getproductsdetalies(String prodid) {
        ...
        DatabaseReference proref= FirebaseDatabase.getInstance().getReference().child("Products").child("Category").child("Dresses").child("Simple Dresses");
